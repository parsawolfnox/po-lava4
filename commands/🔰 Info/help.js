...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    if (args[0]) {
      const embed = new MessageEmbed().setColor(es.wrongcolor).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null);
      ...
      let OverviewEmbed = new MessageEmbed()
        ...
        .addField(":muscle: **__My Features__**",
          `>>> :notes: An advanced <:846090652231663647:965257997972545617> **Music System** with **Audio Filtering**
...
<:900389724936609842:967806376636211210> Made by [**PARSAWOLF Development**](https://discord.gg/uxGHwJV4xj)`)
    ...
    function allotherembeds_eachcategory(filterdisabled = false) {
      ...
      var embed0 = new MessageEmbed()
        ...
        .addField(`<:787321652345438228:967807266516521000> **Server Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField(`<:835928340715012137:967807284975648848> **Bot Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "bot").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed0)
      ...
      var embed3 = new MessageEmbed()
        ...
        .addField("<a:859459305152708630:967808323250454618> **Song Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("song")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<:835928340715012137:967807284975648848> **Bot Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed3)
      ...
      var embed5 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:835928340715012137:967807284975648848> **Bot Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      ...
      var embed9 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:787321652345438228:967807266516521000> **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("info")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<:835928340715012137:967807284975648848> **Adjust the Bot**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed9)
