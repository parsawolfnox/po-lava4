  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    collector.on('collect', async button => {
      if (button.user.id === cmduser.id) {
        ...
        content: `<:no:1051442588240064573> You are not allowed to do that! Only: <@${cmduser.id}>`,
        ephemeral: true
      });
    });
    ...
    collector.on('end', collected => {
      tempmsg.edit({
        ...
        components: [],
        content: `${collected && collected.first() && collected.first().customId ? `<:yes:1051442586512019497> **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`
      })
    });
  },
};
